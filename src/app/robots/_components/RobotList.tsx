    ...
    <div className="robot-list-container">
      ...
      {showStats && (
        <Row gutter={[16, 16]} style={{ marginBottom: 20 }}>
          <Col xs={24} sm={12}>
            <Card bordered={false} style={{ borderRadius: 10, boxShadow: '0 2px 6px rgba(0,0,0,0.04)' }}>
              <Statistic
      ...
      {isLoading ? (
        ...
        <Card bordered={false} style={{ borderRadius: 10, textAlign: 'center', padding: '40px 0' }}>
          <Empty
          ...
          {robots.map((robot) => (
            <Col xs={24} sm={12} lg={8} key={robot._id || robot.id || robot.robot_id}>
              <RobotItem
                ...
                onClick={
                  onSelectRobot ||
                  ((r) => router.push(`/robots/${r.id || r.robot_id}`))
                }
              />
            </Col>
          ))}
